...
class AlarmLeaseTests(unittest.TestCase):
    ...
    def test_raise_refresh_resolve_and_expiry_follow_incident_lease(self) -> None:
        ...
        self.assertEqual(self.controller.outputs[-1], (False, True, False, True))

        # 首次蜂鸣结束后，refresh 只能续租，不能重新累计蜂鸣。
        self.clock.value = 0.6
        self.runner.tick()
        self.assertEqual(self.controller.outputs[-1], (False, True, False, False))
        self.clock.value = 2.0
        self.runner.apply(incident_id="incident-low", severity="low", action="refresh")
        self.clock.value = 2.1
        self.runner.tick()
        self.assertEqual(self.controller.outputs[-1][3], False)

        # 重复 raise 也只续租；同一事故不能再次蜂鸣。
        self.runner.apply(incident_id="incident-low", severity="low", action="raise")
        self.clock.value = 2.2
        self.runner.tick()
        self.assertEqual(self.controller.outputs[-1][3], False)

        # 新的高等级事故取得控制权并首次蜂鸣；解除后自动降回低等级且不补蜂鸣。
        # 选择一个闪烁 on 相位，便于同时断言三色灯组合。
        ...
        self.assertEqual(self.controller.outputs[-1], (True, True, True, True))
        self.runner.apply(incident_id="incident-high", severity="high", action="resolve")
        self.runner.tick()
        self.assertEqual(self.runner.status()["effective_severity"], "low")
        self.assertEqual(self.controller.outputs[-1], (False, True, False, False))

        # low 的最后续租截止后，所有输出必须自动关闭。
        self.clock.value = 5.3
        self.runner.tick()
